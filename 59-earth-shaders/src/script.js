...
// for the image textures we use SRGB, but not for the data texture (specular) - that one is linear
...
const earthAtmosphere = new THREE.Mesh(earthGeometry, atmosphereMaterial);
earthAtmosphere.scale.set(1.04, 1.04, 1.04);
scene.add(earthAtmosphere);
/**
 * Sun
 */
const sunSphericalCoords = new THREE.Spherical(1, Math.PI * 0.5, 0.5);
...
const updateSun = () => {
  sunDirection.setFromSpherical(sunSphericalCoords);
...
gui.add(sunSphericalCoords, "phi").min(0).max(Math.PI).onChange(updateSun);
gui
  .add(sunSphericalCoords, "theta")
  .min(0)
  .max(3 * 2 * Math.PI)
  .onChange(updateSun);
